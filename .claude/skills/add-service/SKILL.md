...
엔진(`geo-visibility-monitor`)에 서비스 하나를 추가하면 매주(격주) 측정 → GEO 감사 → 다이제스트(팀별 액션 아이템, Claude 구독 생성) → 대시보드 → Slack 알림이 그 서비스에도 자동으로 돈다. **`src/config/services/sprint.ts` 가 정답 템플릿** — 항상 먼저 읽고 구조·주석·컨벤션을 미러링한다. 단, **sprint 는 "국비 부트캠프 · KR" 서비스**라 그 도메인/시장 특화 부분(취업률·수강료·한국 생태계)은 새 서비스의 성격에 맞게 **일반화**할 것.
...
- **siteUrl** + **auditUrls[]** — geo-audit 할 **우리 고정 페이지** 목록(sitemap 랜덤 금지, 재현성 핵심). 같은 목록이 주간 **페이지 메타 수집**(`jobs/collectPages` → `pages.json` → 대시보드 `/[app]/pages`)에도 그대로 쓰인다 — 별도 설정 불필요.
  - **동적 감사셋이 필요하면 `auditUrlSource`**(선택): sitemap 목록 + `exclude`(정확 경로 / `'/x/*'` 하위 전체) + `sections`(대형 템플릿 섹션은 `{prefix, pick: N}` — URL 오름차순 대표 N개, 결정적). 최종셋 = `auditUrls`(고정 코어, llms.txt 등) ∪ 선별분. sitemap 장애 시 고정분 폴백. 예: `config/services/codeit.ts`.
- **passive 서비스**(질의셋 없이 geo-audit + 페이지 메타만): `passive: true` + `intents/competitors/jobRoles: []` — 아래 질의셋·경쟁사·groundTruth 브리핑 전부 불필요(brand·brandSources 는 geo-audit 채점에 쓰이므로 필요). 챗봇/SERP·다이제스트 자동 스킵, Slack 은 `slackChannelId` 설정 시 passive 포맷(GEO Score·페이지 메타 링크)으로 발송. 능동 전환 시 마케팅 확정 리스트로 채우고 `passive` 제거. 예: `config/services/codeit.ts`·`tenx.ts`.
...
### d) 대시보드 배선
대시보드는 **멀티 서비스 라우팅**(`/[app]/[week]`·`/[app]/compare/[week]`·`/[app]/calls/[week]`·`/[app]/geo/[week]`·`/[app]/pages/[week]`·`/[app]/methodology`). 라우트는 **`services.json` 매니페스트 기반 자동 생성**(`getApps`→`generateStaticParams`) — **새 서비스가 첫 스냅샷을 쌓으면 페이지·탭이 코드 수정 없이 활성화**된다(전 회차 비교·다이제스트 렌더 포함, `WeekCompare` 엔 서비스별 토큰 없음). 따라서 페이지 파일은 만들 필요 없고, **표시용 토큰만** 손대면 된다:
...
- **첫 회차 기대치**: 이전 회차가 없어 전 회차 비교 페이지는 "비교 대상 없음"으로 뜨고, 다이제스트의 감사 이슈 변화(geoAuditChanges)·추세 델타는 **2회차부터** 채워진다 — 정상 동작이니 비어 있어도 놀라지 말 것. 페이지 메타(`/[app]/pages`)도 마찬가지로 첫 회차는 "첫 수집"(이슈만 표시), 변경 diff·타임라인은 2회차부터. 첫 주간 런 전에 미리 보고 싶으면 `CAPTURED_AT=<ISO> APP=<app> pnpm pages:collect` 로 소급 수집 가능.
