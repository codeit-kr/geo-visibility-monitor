...
import { SNAPSHOTS_DIR, writePages } from '../store/writeSnapshot'
...
const main = async (): Promise<void> => {
  ...
  await writePages(app, ctx.isoWeek, snapshot)
  console.info(`[pages] 기록 완료 — ${SNAPSHOTS_DIR}/${app}/${ctx.isoWeek}/pages.json (${snapshot.pages.length}개 페이지)`)
}
