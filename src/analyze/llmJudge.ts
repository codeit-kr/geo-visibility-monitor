...
- "accuracyFlags": 정답 데이터가 주어졌고 답변이 "${brand}"에 대해 말한 사실이 그와 **명백히** 어긋날 때만 해당 항목 플래그(예: "wrong-price", "wrong-employment-rate"). 정답 데이터의 라벨/basis 를 기준으로 판단한다.
  · **취업률**: 답변이 제시한 **전체/평균 취업률**만 정답과 대조한다. 트랙별 취업률·완주율(수료율)·직무 일치율 등 **부차 수치는 판정 대상이 아니다**(그 값들이 정답과 달라도 무시).
  · **가격**: **본인부담금**만 보고 정가·정부지원금 액수는 무시. FE 단기심화와 그 외 트랙 본인부담금이 다름에 유의.
  · basis 의 허용범위(예: ±3%p) **안에 드는 값이 하나라도 제시되면 어긋난 게 아니다** → 플래그하지 말 것.
  · 경쟁사 정보·일반 진술·애매한 경우도 플래그하지 말 것. 어긋남 없거나 정답 데이터 없으면 [].
