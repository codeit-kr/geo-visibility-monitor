...
**규칙:** composite 에 반영하는 외부 신호는 `brandSources`(v1) 고정 체크리스트뿐. brandSources 6개는 **전부 "(모회사)" = 코드잇** 항목이며 ascent 전용 엔티티가 아니다.
...
- **Instagram (ascent 전용):** ascent Organization `sameAs` = `https://www.instagram.com/ascent.me`. brandSources(v1)에 미포함이나 **ascent 전용 소셜 채널**로 향후 brandSources 승격 검토 가치. → v2 후보.
