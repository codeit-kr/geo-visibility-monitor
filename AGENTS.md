...
//   PageMeta/PagesSnapshot(auditUrls 페이지 메타 원본 — pages.json, 판정·diff 는 대시보드 렌더 시 계산),
//   RollupIndex/WeekSummary/ServicesManifest(롤업). Group B(Referral)·D(OwnedSurface)는 드롭.
```

Aggregate metrics (Mention Rate, SoV) are **computed from the call pool**, not stored per-call: headline = pool over all `visibility` calls; per-intent = pool over that intent's calls. 비용(cost.json)·원문(responses.json)·페이지 메타(pages.json)는 별도 산출물.
